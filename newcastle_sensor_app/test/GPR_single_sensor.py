...
# The imports used only by the original research/diagnostic functions are
# optional so that importing this module does not stop the Shiny app starting.
try:
...
try:
    ...
    # Only the original optional-prior research functions use TFP.
    tfp = None
...
# ============================================================================
# SINGLE-SENSOR GP SHINY INTEGRATION: START
# ============================================================================
#
# This is the only section imported and called by app.py.  It adapts the
# research model below to the app's live Timestamp/Value data format.
#
# Deliberate changes from the original notebook workflow:
#   * accepts one already-selected sensor dataframe;
#   * uses real elapsed time in hours rather than yearly array indices;
#   * uses a deterministic Matern-3/2 + fixed daily-periodic kernel;
...
#
# The original research functions remain below this marked section.


class GPForecastError(RuntimeError):
    """Raised when a single-sensor GP forecast cannot be produced."""


def _app_sampling_interval(timestamps):
...
def forecast_single_sensor_gp(
    readings,
    *,
    forecast_hours=24,
    training_hours=168,
    max_training_points=700,
    ...
    readings : pandas.DataFrame
        Must contain ``Timestamp`` and ``Value``.  The app already supplies
        data for one selected sensor.
    forecast_hours : int or float
        Length of the future forecast.  The app uses 24 hours.
    training_hours : int or float
        Maximum recent history used for fitting.  The app uses seven days.
    max_training_points : int
        Safety limit for the cubic cost of exact Gaussian-process fitting.
    optimiser_iterations : int
        Maximum number of SciPy optimisation iterations.

    Returns
    -------
    pandas.DataFrame
        Columns: ``Timestamp``, ``prediction``, ``lower_95`` and ``upper_95``.
    """

    if gpflow is None or tf is None:
        detail = f" ({_GP_IMPORT_ERROR})" if _GP_IMPORT_ERROR else ""
        raise GPForecastError(
            "GPflow and TensorFlow are required for the GP forecast" + detail
        )
    ...
    if missing:
        raise GPForecastError(
            "Missing GP input columns: " + ", ".join(sorted(missing))
        )
    ...
    data = (
        data.dropna(subset=["Timestamp", "Value"])
    ...
    if len(data) < 12:
        raise GPForecastError(
            "At least 12 valid readings are required for the GP forecast."
        )

    interval, frequency = _app_sampling_interval(data["Timestamp"])
    regular = (
        data.set_index("Timestamp")["Value"]
        .resample(frequency)
        .mean()
        .dropna()
    )
    ...
    if len(training) < 12:
        raise GPForecastError(
            "Too few regular observations remain after preparing the GP data."
        )
    ...
    x_train = (
        (training.index - origin) / pd.Timedelta(hours=1)
    ...
    trend_kernel = gpflow.kernels.Matern32(
        variance=0.7,
        lengthscales=24.0,
    )
    daily_base = gpflow.kernels.SquaredExponential(
        variance=0.3,
        lengthscales=3.0,
    )
    daily_kernel = gpflow.kernels.Periodic(
        base_kernel=daily_base,
        period=24.0,
    )
    ...
    try:
        ...
        raise GPForecastError(f"GP optimisation failed: {exc}") from exc

    forecast_steps = max(
        1,
        int(np.ceil(pd.Timedelta(hours=float(forecast_hours)) / interval)),
    )
    forecast_times = pd.date_range(
        start=latest_time + interval,
        periods=forecast_steps,
        freq=frequency,
    )
    x_forecast = (
        (forecast_times - origin) / pd.Timedelta(hours=1)
    ).to_numpy(dtype=np.float64).reshape(-1, 1)
...
# ============================================================================
# SINGLE-SENSOR GP SHINY INTEGRATION: END
# ============================================================================
